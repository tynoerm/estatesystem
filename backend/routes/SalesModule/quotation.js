...
router.route("/quotation/latest-quote", async (req, res) => {
  try {
    const latestQuote = await Quotation.findOne().sort({ quoteNumber: -1 });
    const latestQuoteNumber = latestQuote ? latestQuote.quoteNumber : 0;
    res.json({ latestQuoteNumber });
  } catch (error) {
    res.status(500).json({ error: "Error fetching latest quote number" });
  }
});
